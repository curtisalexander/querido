...
`profile` is statistical (min/max/mean/null_pct/distinct_count — use when you
want *distributions*). `quality` is invariant-oriented (elevates columns
whose null rate, uniqueness, or stored `valid_values` enum is violated — use
when you want *what's wrong*). They're complementary; run both on a new
table.
